...
class Dia:
    ...
    def load_audio(self, audio_path: str):
        """Load an audio file and encode it with the DAC model.
        ...
        # Load waveform with soundfile to avoid TorchCodec dependency
        data, sr = sf.read(audio_path, always_2d=True)  # (T, C)
        audio = torch.from_numpy(data.T).float()        # (C, T)
        ...
        # Encode with DAC (returns the [T, C] representation Dia expects)
